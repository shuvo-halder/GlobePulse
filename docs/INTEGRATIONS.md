...
- **Entity Extraction & Intelligence Entity Foundation:** Entity extraction executes strictly after enrichment and prior to database persistence. Extracted entities (`domain`, `ip`, `url`, `country`, `location`, `infrastructure`) are canonicalized using deterministic normalization rules (such as RFC 5952 IPv6 compression, lowercase FQDN without port, deterministic URL query parameter sorting, and ISO-3166 alpha-2 country codes). Entities are globally unique by `(entity_type, normalized_value)` in PostgreSQL, enabling multi-event entity correlation, while `threat_event_entities` captures event-specific extraction metadata, extraction method (`structured`, `deterministic`, `regex`), confidence, and source field provenance.
